...
module Flags = struct
  ...
  let username =
    map
      ...
      (flag "username" (required string) ~doc:"STRING - your username, who to play as.")
  ;;
  ...
  let against =
    map
      ...
      (flag
         ...
         (optional string)
         ~doc:
           "Easy | Medium | Hard - Difficulty of the server to play against. If unset \
            will leave game open for someone else to join.")
  ;;

  let game_kind =
    map
      ~f:(Fn.compose Game_kind.t_of_sexp Sexp.of_string)
      (flag "game-kind" (required string) ~doc:"Tic_tac_toe | Omok - Difficulty of ")
  ;;

  let refresh_rate =
    flag
      "refresh-rate"
      (optional_with_default (Time_float.Span.of_sec 0.1) Time_float_unix.Span.arg_type)
      ~doc:"How often to poll for new game state"
  ;;

  let game_id =
    map ~f:Game_id.of_int (flag "game-id" (required int) ~doc:"INT - Game ID to join.")
  ;;
end
...
let create_client ~host ~port =
  let uri =
    Uri.empty |> Fn.flip Uri.with_host (Some host) |> Fn.flip Uri.with_port (Some port)
  in
...
let cmd_join_game =
  Command.async_or_error
    ~summary:
      "Send off an rpc to join a game and then randomly places pieces using web socket \
       rpc"
    (let%map_open.Command port =
     ...
     fun () ->
       let open Deferred.Or_error.Let_syntax in
       let%bind client = create_client ~host ~port in
       Rpc_client.join_game_and_play client ~me:username ~game_id ~game_ai ~refresh_rate)
;;

let cmd_play_against_self =
  Command.async_or_error
    ~summary:"Send off an rpc to create and then immediately start playing against itself"
    (let%map_open.Command port =
       flag "port" (optional_with_default 8181 int) ~doc:"port on which to serve"
     and host = flag "host" (required string) ~doc:"The host to connect to"
     and username = Flags.username
