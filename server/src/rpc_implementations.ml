...
let create_game ~global_state =
  Rpc.Rpc.implement'
    ...
    (fun (user_state : User_state.t) { txt = game_proposal; username } ->
       let { Create_game.Query.game_kind; against_server_bot } = game_proposal in
       let against =
         match against_server_bot with
         | None -> "human player"
         | Some difficulty -> [%string "server bot on %{difficulty#Difficulty} difficulty"]
       in
       info
         [%string
           "%{user_state.user} as (%{username#Username}) proposed '%{game_kind#Game_kind} \
            against %{against}'"];
       State.create_game global_state ~first_player:username ~query:game_proposal)
;;
...
let join_existing_game ~global_state =
  Rpc.Rpc.implement'
    ...
    (fun (user_state : User_state.t) { txt = game_id; username } ->
       info
         [%string
           "%{user_state.user} as (%{username#Username}) wants to join '%{game_id#Game_id}'"];
...
let show_all_games_with_two_players ~global_state =
  Rpc.Rpc.implement'
    ...
    (fun (user_state : User_state.t) () ->
       info [%string "%{user_state.user} wants to view all games with two players"];
       State.get_all_running_games global_state)
;;

let list_all_joinable_games ~global_state =
  Rpc.Rpc.implement' List_all_joinable_games.rpc (fun (user_state : User_state.t) () ->
...
let take_turn ~global_state =
  Rpc.Rpc.implement
    Take_turn.rpc
    (fun (user_state : User_state.t) { txt = { game_id; position }; username } ->
       info
         [%string
...
let is_thinking ~global_state =
  Rpc.Rpc.implement' Is_thinking.rpc (fun (user_state : User_state.t) game_id ->
    info
      [%string
        "%{user_state.user}  wants to know if game id '%{game_id#Game_id}' is thinking."];
