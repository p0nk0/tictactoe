...
let score ~(me : Piece.t) ~(game_kind : Game_kind.t) ~(pieces : Piece.t Position.Map.t)
  : float
  =
  let my_score = Traverse.score_for_piece ~piece:me ~game_kind pieces in
  let their_score = Traverse.score_for_piece ~piece:(Piece.flip me) ~game_kind pieces in
...
let empty_pieces ~game_kind ~pieces =
  ...
  let all_positions =
    ...
    { Position.row; column }
  in
  List.filter all_positions ~f:(fun position -> not (Map.mem pieces position))
;;
...
let adjacent_pieces_or_default ~game_kind ~pieces ~default =
  ...
  | true -> [ default ]
  | false ->
    Map.fold pieces ~init:Position.Set.empty ~f:(fun ~key:position ~data:_ acc ->
...
let next_possible_moves ~(game_kind : Game_kind.t) ~(pieces : Piece.t Position.Map.t)
  : Position.t list
  =
...
let%test_module "Next possible moves" =
  (module struct
    let%expect_test "tictactoe" =
      let pieces =
        Position.Map.of_alist_exn [ { Position.row = 0; column = 0 }, Piece.X ]
      in
     ...
     | false ->
       ...
       (match possible_moves with
        ...
        | _ :: _ ->
          ...
          in
          let should_cutoff : value:float -> alpha:float -> beta:float -> bool =
            match am_maximizing_player with
          ...
          let out =
            List.fold_until
              ...
              ~f:(fun (acc_score, alpha, beta) possible_move ->
                ...
                | false ->
                  let alpha, beta = update_alpha_beta ~value:score ~alpha ~beta in
...
let compute_next_move ~depth ~me ~(game_state : Game_state.t) : Position.t =
  let next_possible_moves =
    next_possible_moves ~game_kind:game_state.game_kind ~pieces:game_state.pieces
  in
  match next_possible_moves with
  | [] -> raise_s [%message "no possible moves!" (game_state : Game_state.t)]
  | [ single ] -> single
  | first :: (_ :: _ as rest) ->
    (* CR-someday jrodriguez: I think that if I had implemented ab-pruning from the start,
       it _could_ be cleaner. This is sooo messy. *)
