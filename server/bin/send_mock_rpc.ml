    ...
    (let%map_open.Command port =
     ...
     fun () ->
       ...
       let%bind join_game_response =
         Rpc_client.join_existing_game
           client
           ~query:{ username; txt = game_id }
       in
       ...
       Deferred.Or_error.repeat_until_finished () (fun () ->
           ...
           (match game_state.game_status with
            | Game_status.Game_over { winner } ->
              let winner =
                Option.map
                  winner
                  ~f:(fun (winning_piece, _winning_positions) ->
            ...
            | Game_status.Turn_of _ ->
              ...
              let position =
                let all_positions =
                  let board_length =
                    Game_kind.board_length game_state.game_kind
                  in
                ...
                |> Option.value_exn
                     ~message:"Whoops, no more available spots"
              in
              let%bind.Deferred () =
                Clock.after (Time_float.Span.of_sec 0.1)
              in
              let%bind response =
                Rpc_client.take_turn
                  ...
                  ~query:
                    { username
                    ; txt = { position; game_id = game_state.game_id }
                    }
              in
              (match response with
         ...
         | Get_game.Response.Waiting_for_someone_to_join _ ->
           print_s
             [%message
               "Whoops, expected the game to have started, but found a \
                joinable game instead."];
           return (`Finished ())
